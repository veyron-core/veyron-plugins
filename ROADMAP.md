...
| `system` | **P1 implemented** (branch `feat/system-plugin`): read-only `sys_info`/`sys_battery`(UPower)/`sys_procs`/`sys_volume`(wpctl→pactl), backend-trait detection, `ERR_SYS_*` taxonomy, 24 tests — see `plugins/system/ROADMAP.md`. Next: reversible setters `sys_volume_set`/`sys_volume_mute`, `sys_brightness_set`, `sys_lock`, power profile (P2); macOS subset (P3). One domain: local system state. Destructive or differently-scoped actions (kill process, network config, device management) deliberately out — they'd turn this into shell-lite | — | `PERMISSION_SYSTEM` (existing) — broad access, keep strict |
