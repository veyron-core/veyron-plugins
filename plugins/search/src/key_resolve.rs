        ...
        .await
    {
        Ok(resp) if resp.status == ActionStatus::ActionOk as i32 => {
            match serde_json::from_slice::<SecretGetResponse>(&resp.data_json) {
                ...
                Ok(_) => {
                    eprintln!(
                        "[search] secrets vault has no non-empty value for '{handle}', \
                         falling back to env var '{handle}'"
                    );
                }
                Err(e) => {
                    eprintln!(
                        "[search] secrets vault returned a malformed reply for '{handle}' \
        ...
        Ok(resp) => {
            eprintln!(
                "[search] secrets vault unavailable for '{handle}' ({}), \
                 falling back to env var '{handle}'",
                resp.error
            );
        }
        Err(e) => {
            eprintln!(
                "[search] secrets vault unavailable for '{handle}' ({e}), \
                 falling back to env var '{handle}'"
            );
        }
    }
